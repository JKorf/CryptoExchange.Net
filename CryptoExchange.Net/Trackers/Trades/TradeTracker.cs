...
public class TradeTracker : ITradeTracker
{
    ...
    /// <inheritdoc />
    public SharedTrade[] GetData(DateTime? fromTimestamp = null, DateTime? toTimestamp = null)
    {
        lock (_lock)
        {
            ApplyWindow(true);
            ...
            if (fromTimestamp != null)
                result = result.Where(d => d.Timestamp >= fromTimestamp);
            if (toTimestamp != null)
                result = result.Where(d => d.Timestamp <= toTimestamp);
