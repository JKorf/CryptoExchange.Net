...
public class KlineTracker : IKlineTracker
{
    ...
    private static KlinesStats GetStats(IEnumerable<SharedKline> klines)
    {
        if (!klines.Any())
    ...
    /// <inheritdoc />
    public SharedKline[] GetData(DateTime? fromTimestamp = null, DateTime? toTimestamp = null)
    {
        lock (_lock)
        {
            ApplyWindow(true);
            ...
            if (fromTimestamp != null)
                result = result.Where(d => d.OpenTime >= fromTimestamp);
            if (toTimestamp != null)
                result = result.Where(d => d.OpenTime <= toTimestamp);
