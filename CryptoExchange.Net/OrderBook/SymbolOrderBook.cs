...
public abstract class SymbolOrderBook : ISymbolOrderBook, IDisposable
{
    ...
    {
        var highest = Math.Max(bids.Length != 0 ? bids.Max(b => b.Sequence) : 0, asks.Length != 0 ? asks.Max(a => a.Sequence) : 0);
        var lowest = Math.Min(bids.Length != 0 ? bids.Min(b => b.Sequence) : long.MaxValue, asks.Length != 0 ? asks.Min(a => a.Sequence) : long.MaxValue);
    ...
    public string ToString(int rows)
    {
        ...
        {
            var ask = book.asks.Length > i ? book.asks[i]: null;
            var bid = book.bids.Length > i ? book.bids[i]: null;
